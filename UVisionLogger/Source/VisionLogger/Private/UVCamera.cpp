...
void AUVCamera::BeginPlay()
{
	...
	if (bFirstPersonView) {
		...
		UGameplayStatics::GetAllActorsOfClass(GetWorld(), AMCPawn::StaticClass(), VRPawns);
		UsedVRCamera = Cast<AMCPawn>(VRPawns[0])->VRCamera;
	}
...
void AUVCamera::Tick(float DeltaTime)
{
	...
	if (bFirstPersonView) {
		ColorImgCaptureComp->SetWorldLocationAndRotation(UsedVRCamera->GetComponentLocation(), UsedVRCamera->GetComponentRotation());
	}
